...
$supplierquer = mysqli_query($conn, "SELECT * FROM supplier");
?>
<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="container mx-auto p-8">
        ...
        <div class="printable">
            <h3 class="text-2xl font-bold text-center mb-6">Detail Supplier</h3>
            <table class="min-w-full border">
                <thead class="bg-gray-800 text-white">
                    <tr>
                        <th class="py-3 px-4 border">No.</th>
                        <th class="py-3 px-4 border">Nama Supplier</th>
                        <th class="py-3 px-4 border">Alamat</th>
                        <th class="py-3 px-4 border">No. Telp</th>
                    </tr>
                </thead>
                <tbody>
                    <?php 
                    $i = 1; 
                    foreach($supplierquer as $row) : 
                    ?>
                    <tr>
                        <td class="py-3 px-4 border"><?= $i++; ?></td>
                        <td class="py-3 px-4 border"><?= $row["nama"]; ?></td>
                        <td class="py-3 px-4 border"><?= $row["alamat"]; ?></td>
                        <td class="py-3 px-4 border"><?= $row["phone"]; ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
